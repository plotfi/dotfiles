...
cd
cd test-suite-build
cmake -DLLVM_BUILT_ROOT=../bootstrap -DCMAKE_SYSROOT=../sysroots/$ARCH -DSTDLIBCXX_VERSION=8 \
      -C../sysroots/$ARCH/CMakeCache.txt -C../test-suite/cmake/caches/O3.cmake  ../llvm-test-suite/
make -j8
